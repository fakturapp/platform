...
export default function DashboardHomePage() {
  const { user } = useAuth()
  const { keys, openCreate } = useApiKeys()
          ...
          <Card className="border-border/50 transition-colors hover:border-accent/40 hover:bg-surface-hover">
            <CardContent className="p-5">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Créer, roter, révoquer
              </p>
            </CardContent>
          </Card>
        </Link>

        <button type="button" onClick={openCreate} className="block text-left">
          <Card className="border-border/50 transition-colors hover:border-accent/40 hover:bg-surface-hover">
            <CardContent className="p-5">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-accent-soft text-accent">
          ...
          <CardContent className="p-0">
            {recentKeys.length === 0 ? (
              <div className="px-5 py-8 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Vos clés API actives apparaîtront ici dès le premier appel.
                </p>
